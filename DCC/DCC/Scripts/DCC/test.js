...
function insertRecord(i) {
    ...
    if (inx < 0)
        ...
        var Data = {
            'clientSessionTherapyId': i,
            'clientId': $('#TCLSVID').val(),
            'providerId': $('#providerId').val(),
            'serviceInfo': $('#cSTId' + i + ' .service').val(),
            'locationInfo': $('#cSTId' + i + ' .location').val(),
            'svc': $('#cSTId' + i + ' .service option:selected').text(),
            'startEndDates': $('#weekSelect').val(),
            'serviceDate': dateObj.val(),
            'inOffsetMinutes': inx,
        ...
        $.ajax({
            type: 'POST',
            url: srvcUrl + '/Home/InsertProviderTimeSheetRecord',
