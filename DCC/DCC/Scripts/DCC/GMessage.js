  ...
  if (channel.isGroupChannel()) {
    ...
    members.forEach(function (member) {
      if (member.connectionStatus == "online") {
        ...
        var dateTimeString =
          dateTime.getMonth() + 1 + "/" + dateTime.getDate() + " " + dateTime.getHours() + ":" + dateTime.getMinutes();
      }
      ...
      memberListHtml +=
        ...
        isOnline +
        '">' +
        (member.nickname.length > 30
          ? xssEscape(member.nickname.substring(0, 30)) + "..."
          : xssEscape(member.nickname)) +
        "  </div>" +
        "</div>";
    });
...
function getUserListLoop(UserListQuery) {
  if (UserListQuery.hasNext) {
    UserListQuery.next(function (userList, error) {
      ...
      getUserListLoop(UserListQuery);
    });
  } else {
    console.log("length", Object.keys(UserList).length);
    return;
  }
}
...
function userClick(obj) {
  ...
  if ((selectCount > 1 || (selectCount == 1 && flag_mp == 1)) && !(selectCount == 2 && flag_mp == 0)) {
    if (flag_mp == 1) selectCount++;
    else if (flag_mp == 0) selectCount--;
  ...
  if (message) {
    ...
    if (lastMessageDateString) {
      lastMessageDateString =
        '<div><img src="images/icon-time.png" style="vertical-align: moddle; padding-bottom: 2px;" /> <span> ' +
        lastMessageDateString +
        "</span></div>";
    }

    $(".left-nav-channel-group[data-channel-url=" + message.channelUrl + "] .left-nav-channel-lastmessage").html(
      lastMessage
    );
    ...
    $(".left-nav-channel-group[data-channel-url=" + message.channelUrl + "] .left-nav-channel-lastmessagetime").html(
      lastMessageDateString
    );
  }
}
  ...
  if (addFlag && !isGroup) {
    ...
    $("#messaging_channel_list").append(
      '<div class="left-nav-channel left-nav-channel-group left-nav-channel-group--active" ' +
        ...
        '"' +
        ">" +
        '<div class="left-nav-channel-members">' +
        channelMemberList +
        "</div>" +
...
function joinGroupChannel(channelUrl, callback) {
  ...
  $.ajax({
    type: "GET",
    url:
      "https://api-F742C54C-036E-4DD0-B74D-E59E38ED7B30.sendbird.com/v3/users/" +
      userId.toString() +
    ...
    success: function (r) {
      ...
      else $(".message__unread").html(totalUnreadMessage);
      if (totalUnreadMessage == 0) $(".message__unread").remove();
    },
    error: function (r) {},
    //timeout: 10000
  });
  ...
  sb.GroupChannel.getChannel(channelUrl, function (channel, error) {
    ...
    $(".chat-header .header-icon > img").attr("src", "images/user_icon.png");
  ...
  if (message.isAdminMessage()) {
    ...
    if (isCurrentUser(user.userId)) {
      ...
      msgList += msg
        ...
        .replace("%nickname%", xssEscape(user.nickname))
...
function fileMessageList(message) {
  ...
  if (isCurrentUser(user.userId)) {
    msgList +=
      ...
      "  </label>" +
      '  <label class="chat-canvas__list-separator">:</label>' +
      '  <label class="chat-canvas__list-text" data-messageid="%messageid%">'.replace(
        "%messageid%",
        message.messageId
      ) +
      '    <label class="chat-canvas__list-text-file">FILE</label>' +
      '    <a href="' +
      xssEscape(message.url) +
      '" target="_blank">' +
...
function imageMessageList(obj) {
  ...
  if (isCurrentUser(user.userId)) {
    msgList +=
      ...
      "  </label>" +
      '  <label class="chat-canvas__list-separator">:</label>' +
      '  <label class="chat-canvas__list-text"  onclick="window.open(\'' +
      xssEscape(message.url) +
      "', '_blank');\" data-messageid=\"%messageid%\">".replace("%messageid%", message.messageId) +
      xssEscape(message.name) +
      "  </label>" +
      '  <div class="chat-canvas__list-file" onclick="window.open(\'' +
      xssEscape(message.url) +
      "', '_blank');\">" +
      '<img src="' +
      xssEscape(message.url) +
      '" class="chat-canvas__list-file-img" onload="afterImageLoad(this)">' +
      "  </div>" +
      "</div>";
  } else {
    msgList +=
...
function addUnreadCount(item, unread, targetUrl) {
  ...
  if ($(item).find('div[class="left-nav-channel__unread"]').length != 0) {
    ...
    $('<div class="left-nav-channel__unread">' + unread + "</div>").prependTo(
      ".left-nav-channel-group[data-channel-url=" + targetUrl + "]"
    );
  }
}
