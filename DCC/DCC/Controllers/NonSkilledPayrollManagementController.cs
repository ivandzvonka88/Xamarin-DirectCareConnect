...
namespace DCC.Controllers
{
    ...
    public class NonSkilledPayrollManagementController : DCCBaseController
    {
        private readonly SQLHelper sqlHelper;

        public NonSkilledPayrollManagementController()
        {
        ...
        public async Task<ActionResult> GetPayollReport(int payrollId)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    using (SqlConnection cn = new SqlConnection(UserClaim.conStr))
                    {
                        SqlCommand cmd = new SqlCommand("sp_ProvidersAllPayrollHCBSGet", cn)
                        {
                            CommandType = CommandType.StoredProcedure
                        };
                ...
                foreach (DataRow dr in Providers.Rows)
                {
                    ...
                    {
                        payrollItemList += ",HCBS," + (string)drv["PayrollCode"] + "," + Convert.ToDouble(drv["Units"]);
        ...
        public async Task<ISolvedPayrollStatus> GetISovledPayrollStatus(int payrollId)
        {
            ...
            try
            {
                ...
                int iSolvedID = (int)ds.Tables[0].Rows[0].ItemArray[1];
        ...
        public async Task<ISolvedPayrollStatus> InitiateISolvedPayrollPreview(int payrollId)
        {
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    using (SqlConnection cn = new SqlConnection(UserClaim.conStr))
                    {
                        SqlCommand cmd = new SqlCommand("sp_SetISolvedIDNonSkilled", cn)
                        {
                            CommandType = CommandType.StoredProcedure
                        };
        ...
        public async Task<ActionResult> GetPayollExport(int payrollId)
        {
            ...
            try
            {
                ...
                string companyCode = (string)drx["nonSkilledPayrollId"];
